...
const int32_t kAudioCaptureBufferSize = 10000;
const int32_t i2s_bytes_to_read = kMaxAudioSampleSize*2;
...
uint8_t i2s_read_buffer[i2s_bytes_to_read] = {};
  ...
  while (1) {
    // rb_stat(g_audio_capture_buffer);
    ...
    if (bytes_read <= 0) {
      ...
      /* write bytes read by i2s into ring buffer 
        - Don't use portMAX_DELAY to wait */
      int bytes_written = rb_write(g_audio_capture_buffer, (uint8_t*)i2s_read_buffer, bytes_read, 20);
      ...
      if (bytes_written <= 0) {
        // rb_cleanup(g_audio_capture_buffer);
...
TfLiteStatus GetAudioSamples(tflite::ErrorReporter* error_reporter,
  ...
  //        (void*)(i2s_read_buffer),sizeof(g_audio_output_buffer));
//  Serial.println("\n💽 Bytes ----------");
//   for (int i = 0; i < kMaxAudioSampleSize*2; i += 2) {
//   // Serial.printf("%02x%02x ",i2s_read_buffer[i], i2s_read_buffer[i + 1]);
//   g_audio_output_buffer[i / 2] = (i2s_read_buffer[i + 1] << 8 | i2s_read_buffer[i] );
// }
  // Print the contents of the int16_array
  Serial.println("\n🎵 16bit-PCM ----------");
  *audio_samples_size = kMaxAudioSampleSize;
  *audio_samples = g_audio_output_buffer;
  for (int i = 0; i < kMaxAudioSampleSize; i++) {
  Serial.printf("%d ", *(*audio_samples + i));
}
  return kTfLiteOk;
}
