...
TfLiteStatus FeatureProvider::PopulateFeatureData(
    ...
    int32_t time_in_ms, int* how_many_new_slices) {
      // TF_LITE_REPORT_ERROR(error_reporter, "Feature generation start");
  ...
  *how_many_new_slices = slices_needed;
  ...
  if (1) {
    ...
    //      ++new_slice) {
      const int new_step = (current_step - kFeatureSliceCount + 1) + i;
      ...
      uint8_t* new_slice_data = feature_data_ + (i * kFeatureSliceSize);
