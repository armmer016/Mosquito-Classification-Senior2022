...
void loop() {
  ...
  //  Serial.print("Feature before");
  TfLiteStatus feature_status = feature_provider->PopulateFeatureData(
      error_reporter, previous_time, current_time, &how_many_new_slices);
      // delay(300);
      // Serial.print("Feature populate");
  if (feature_status != kTfLiteOk) {
    // Serial.print("Feature error");
  ...
  for (int i = 0; i < kFeatureElementCount; i++) {
    model_input_buffer[i] = feature_buffer[i];
      ...
      // Serial.print(String(model_input_buffer[i])+",");
  ...
  int16_t cur_time = millis()-start_time;
  total_time += cur_time;
  TF_LITE_REPORT_ERROR(error_reporter,
                           "##inference time[%d] :  %dms \n## avg time:  %dms", count, cur_time , total_time/count);
